...
class Node:
    ...
    def expand(self, rewards):
        ...
        p, v = self.pred_model.forward(self.s, rewards)
    ...
    def backup(self, vl, rewards):
        ...
        if vl == 0: #never seen
            #print(rewards)
            rewards = [[sum(rs)/len(rs)] for rs in rewards]
            #print(rewards)
...
class Edge:
    ...
    def expand(self, state, pred_model, dyn_model, options, rewards):
        self.child_node = Node(state, pred_model, dyn_model, options)
        self.child_node.parent_edge = self
        v = self.child_node.expand(rewards)
