...
cdef class Edge:
    ...
    cdef uint16_t _count        # N(s, a)
    cdef double _action_value   # Q(s, a) = W(s, a) / N(s, a)

    cdef object _parent_node
    cdef object _child_node

    def __init__(self, double prior, object parent_node, object child_node):
        self._prior = prior
        self._value_sum = 0.0
    ...
    cpdef void update(self, double value):
        ...
        self._parent_node.increment_counter()
        self._action_value = self._value_sum / self._count

    cpdef uint16_t get_count(self):
        return self._count
    ...
    cpdef double ucb(self, double c):
        cdef double total_count = self._parent_node.get_total_count()
