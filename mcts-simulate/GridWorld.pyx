...
cdef class GridWorld:
    ...
    cpdef object copy(self):
        g = GridWorld(self.grid_map, self.time_limit)

        g.cur_x, g.cur_y = self.cur_x, self.cur_y
        g.goal_x, g.goal_y = self.goal_x, self.goal_y
        g.time_limit, g.cur_t = self.time_limit, self.cur_t

        return g
    
    cpdef (int32_t, int32_t) state(self):
        return (self.cur_x, self.cur_y)
